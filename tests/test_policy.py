...
class PolicyFileTestCase(test.TestCase):
    ...
    def test_modified_policy_reloads(self):
        ...
        self.rules.enforce(empty_credentials, action, self.target)
        with open(self.tmpfilename, "w") as policyfile:
            policyfile.write("""{"example:test": ["false:false"]}""")
        # NOTE(vish): reset stored policy cache so we don't have to sleep(1)
        self.rules.reset()
        self.assertRaises(exception.Forbidden, self.rules.enforce,
...
class PolicyTestCase(test.TestCase):
    ...
    def test_enforce_good_action(self):
        ...
        self.rules.enforce(self.credentials, action, self.target)
    ...
    def test_enforce_http_true(self):
        ...
        target = {}
        result = self.rules.enforce(self.credentials, action, target)
    ...
    def test_templatized_enforcement(self):
        ...
        self.rules.enforce(credentials, action, target_mine)
        self.assertRaises(exception.Forbidden, self.rules.enforce,
    ...
    def test_ignore_case_role_check(self):
        ...
        self.rules.enforce(admin_credentials, lowercase_action, self.target)
        self.rules.enforce(admin_credentials, uppercase_action, self.target)
...
class DefaultPolicyTestCase(test.TestCase):
    ...
    def test_not_found_policy_calls_default(self):
        self.rules.enforce(self.credentials, "example:noexist", {})
